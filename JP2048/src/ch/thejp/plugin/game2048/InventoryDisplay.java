...
public class InventoryDisplay {
	...
	private static Material[] woolColors = new Material[]{
			Material.WHITE_WOOL,
...
//			}
	 ...
	 * @return
	 */
	private int calculateColor(long value){
	    return (int) ((Math.log(value) / Math.log(2)) % 16);
	}
	...
	private ItemStack getTileItems(long value){
		...
		if(config.getGameMode() == GameMode.GM64){
			...
			//Default value
			s = new ItemStack(woolColors[(int) value/4]);
		} else {
			//Default value
			s = new ItemStack(woolColors[calculateColor(value)]);
		}
	...
	public void render(){
		...
		//undo button
		ItemStack undo = (undoable.isUndoable() ? config.getDisplayUndo() : emptyField);;
		contents[COLS-2] = undo;
		if(undo!=null && undo.getType()!=Material.AIR && config!=null){
			changeDisplayName(undo, ChatColor.RED + config.getPhrase("display-undo"));
		}
		//score display
