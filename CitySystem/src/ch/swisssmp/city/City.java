...
public class City {
    ...
    public void promoteCity() {
        ...
        DataSource.getResponse(CitySystemPlugin.getInstance(), "promote_city.php", new String[]{
                ...
                "level_id=" + URLEncoder.encode(newLevelId)
        });
    }

    public HTTPRequest addCitizen(Player player, Player parent, String role) {
        if (isCitizen(player.getUniqueId()) || (!isCitizen(parent.getUniqueId()) && !parent.hasPermission("citysystem.admin")))
            return null;
    ...
    private void setMayor(UUID mayor) {
        ...
        if (prevMayor != null)
            prevMayor.setRank(this.isFounder(this.mayor) ? CitizenRank.FOUNDER : CitizenRank.CITIZEN);
        this.mayor = mayor;
