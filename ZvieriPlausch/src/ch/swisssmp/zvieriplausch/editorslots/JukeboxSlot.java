...
public class JukeboxSlot extends PickItemSlot {
    private final ZvieriArenaEditor view;

    public JukeboxSlot(ZvieriArenaEditor view, int slot) {
        super(view, slot);
        this.view = view;
    }
    ...
    protected ItemStack createPick() {
        ...
        itemBuilder.setMaterial(Material.JUKEBOX);
        itemBuilder.setAmount(1);
        itemBuilder.setDisplayName(this.getName());
        List<String> description = new ArrayList<String>();
        description.add(this.getDescriptionColor() + "Jukebox zuweisen");
        description.add(this.getSuggestActionColor() + "Rechtsklick auf Jukebox");
        itemBuilder.setLore(description);
        ItemStack itemStack = itemBuilder.build();
        ItemUtil.setString(itemStack, "link_zvieriarena", this.view.getArena().getId().toString());
        ItemUtil.setString(itemStack, "zvieritool", "jukebox");
        return itemStack;
    }
    ...
    public String getName() {
        return ChatColor.AQUA + "Jukebox";
    }

    @Override
    protected List<String> getNormalDescription() {
        return Arrays.asList("Jukebox zuweisen");
    }
